...
function renderLogin(app){
  app.innerHTML = `<div class="min-h-screen flex items-center justify-center p-6"><div class="w-full max-w-md bg-white rounded-lg shadow p-6">
    ...
    <div class="text-xs text-slate-500 mt-4"></div>
  </div></div>`;
...
function renderRegistrar(el){
  ...
  function showAssign(){ const content=document.getElementById('tabContent'); if(DATA.students.length===0||DATA.subjects.length===0){ content.innerHTML=`<div class="p-4">No students or subjects. Create them first.</div>`; return; } content.innerHTML=`<div class="grid grid-cols-2 gap-3"><select id="as_student" class="border rounded px-3 py-2"><option value="">-- select student --</option>${DATA.students.map(s=>`<option value="${s.id}">${s.student_no} - ${s.name}</option>`).join('')}</select><select id="as_subject" class="border rounded px-3 py-2"><option value="">-- select subject --</option>${DATA.subjects.map(s=>`<option value="${s.id}">${s.code} ${s.title}</option>`).join('')}</select><div class="col-span-2 flex gap-2"><button id="assignBtn" class="bg-sky-600 text-white px-4 py-2 rounded">Assign</button><button id="unassignBtn" class="bg-red-500 text-white px-4 py-2 rounded">Remove</button></div><div id="as_msg" class="col-span-2 text-sm"></div></div>`;
    document.getElementById('assignBtn').addEventListener('click', ()=>{ const sid=parseInt(document.getElementById('as_student').value); const sub=parseInt(document.getElementById('as_subject').value); if(!sid||!sub) return alert('Select'); const st=DATA.students.find(s=>s.id===sid); st.subjects=st.subjects||[]; if(!st.subjects.includes(sub)) st.subjects.push(sub); saveData(); document.getElementById('as_msg').innerText='Assigned'; showAssign(); });
...
function renderProfessor(el){
  el.innerHTML = `<div class="bg-white p-6 rounded shadow"><h2 class="text-lg font-bold">Professor</h2>
    <div class="mt-3 grid grid-cols-2 gap-3">
      ...
      <select id="prof_subject" class="border rounded px-3 py-2">${DATA.subjects.map(s=>`<option value="${s.id}">${s.code} ${s.title}</option>`).join('')}</select>
      <div class="col-span-2 flex gap-2"><button id="startSess" class="bg-green-600 text-white px-4 py-2 rounded">Start Session (focus input)</button><button id="btnIn" class="bg-sky-600 text-white px-4 py-2 rounded">Tap In</button><button id="btnOut" class="bg-orange-500 text-white px-4 py-2 rounded">Tap Out</button></div>
    </div>
  ...
  const attHtml = DATA.attendance.slice().reverse().map(a=>{ const s=DATA.students.find(x=>x.id===a.student_id)||{}; const subj=DATA.subjects.find(x=>x.id===a.subject_id)||{}; return `<div class="py-1">${a.timestamp} — ${s.student_no||''} ${s.name||''} — ${subj.code||''} — ${a.action}</div>`; }).join('')||'<div>No records</div>'; document.getElementById('attList').innerHTML=attHtml;
}
...
function renderReports(el){
  ...
  document.getElementById('repList').innerHTML = DATA.attendance.slice().reverse().map(a=>{ const s=DATA.students.find(x=>x.id===a.student_id)||{}; const subj=DATA.subjects.find(x=>x.id===a.subject_id)||{}; return `<div class="py-1">${a.timestamp} — ${s.student_no||''} ${s.name||''} — ${subj.code||''} — ${a.action}</div>`; }).join('')||'<div>No attendance</div>';
}
